...
{
    m_client = client;
    //Gentooza 20150930 getRoster() to rosterMAnager(), shall we change m_roster from roster to rostermanager?
    ...
    //connect(m_roster, SIGNAL(rosterChanged(QString)), DEPRECATED
    ...
    connect((const QObject*)m_roster, SIGNAL(presenceChanged(const QString, const QString)),
                    this, SLOT(presenceChangedSlot(const QString, const QString)), Qt::AutoConnection);
    connect((const QObject*)m_roster, SIGNAL(rosterReceived()),
            this, SLOT(parseRoster()) , Qt::AutoConnection);
    connect((const QObject*)m_vCardManager, SIGNAL(vCardReceived(const QXmppVCard&)),
            this, SLOT(vCardRecived(const QXmppVCard&)) , Qt::AutoConnection);
}
